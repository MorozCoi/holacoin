...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Holacoin: 1 days
static const int64 nTargetSpacing = 120; // Holacoin: 2 minute blocks
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "HEY, i'm create my own cripto ;-)";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running FoocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FoocoinMiner terminated\n");
        throw;
    }
}
